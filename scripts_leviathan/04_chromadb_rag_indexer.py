...
def local_chroma_rag_inject():
    ...
    print(f"[*] Transformando {len(archivos)} chunks de texto en Embeddings Vectoriales (Batching: {BATCH_SIZE})...")
    ...
    def flush_batch():
        if not batch_docs:
            return
        try:
            print(f"  -> Ejecutando inyección de lote ({len(batch_docs)} documentos)...")
            collection.add(
            ...
            # Limpieza agresiva para evitar envenenamiento de lote (Bolt Learning)
            batch_docs.clear()
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        try:
            ...
            # Segmentación preventiva (Chroma tiene límite por lote) - Optimización Bolt: split(None, 40001)
