...
# Dynamic relative path resolution from script location
...
# Batching configuration for vectorized insertion speedup
BATCH_SIZE = 20
...
def local_chroma_rag_inject():
    ...
    collection = chroma_client.get_or_create_collection(
        name="tesis_cca", embedding_function=sentence_transformer_ef
    )

    if not os.path.exists(CLEAN_CHUNKS_DIR):
        print(
            f"[!] Directorio {CLEAN_CHUNKS_DIR} vacío. Corre el 02_docs_prep_injector primero."
        )
        return

    # Alphabetical sorting ensures deterministic indexing order across environments
    archivos = sorted(
        [f for f in os.listdir(CLEAN_CHUNKS_DIR) if f.endswith(".txt")]
    )
    ...
    print(
        f"[*] Transformando {len(archivos)} chunks de texto en Embeddings Vectoriales (Batch Size: {BATCH_SIZE})..."
    )
    ...
    def flush_batch():
        if not batch_docs:
            return
        try:
            # Batch insertion reduces SQLite write overhead and vector embedding call latency
            collection.add(
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        # Performance Optimization: maxsplit=40001 avoids full-string splitting into millions of list elements
        parts = contenido.split(None, 40001)
        if len(parts) > 40000:
            print(
                f"  [!] Advertencia: {archivo} es enorme. Cortando por limite interno de Chroma."
            )
        ...
        if len(batch_docs) >= BATCH_SIZE:
            print(
                f"  -> [{i}/{len(archivos)}] Incrustando lote de {len(batch_docs)} chunks..."
            )
            flush_batch()
    ...
    print(
        "🎯 Ahora puedes consultar a Claude o ChatGPT usando búsqueda de similitud por cosenos local."
    )
