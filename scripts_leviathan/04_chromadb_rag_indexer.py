...
def local_chroma_rag_inject():
    ...
    def flush_batch():
        if not batch_docs:
            return
        try:
            print(f"  -> Indexando lote de {len(batch_docs)} documentos...")
            collection.add(
            ...
            print(f"  [X] Error indexando lote: {e}")
        finally:
            batch_docs.clear()
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        # Segmentación preventiva eficiente (Chroma tiene límite por lote)
        # Optimizamos split() para no procesar todo el string dos veces
