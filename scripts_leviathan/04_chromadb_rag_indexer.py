...
def local_chroma_rag_inject():
    ...
    # Batch size de 20 para acelerar inserciones hasta 20x en ChromaDB (disminuye overhead de transacciones)
    BATCH_SIZE = 20
    batch_docs = []
    batch_metas = []
    batch_ids = []
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        # Segmentación preventiva eficiente: maxsplit=40000 evita dividir cadenas masivas innecesariamente
        ...
        if len(batch_docs) >= BATCH_SIZE:
            try:
                print(f"  -> Inserción en lote ({len(batch_docs)} docs)...")
                collection.add(
    ...
    # Flush del último lote pendiente
    if batch_docs:
        try:
            print(f"  -> Inserción en lote final ({len(batch_docs)} docs)...")
            collection.add(
                documents=list(batch_docs),
                metadatas=list(batch_metas),
                ids=list(batch_ids)
