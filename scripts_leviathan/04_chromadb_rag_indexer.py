...
BATCH_SIZE = 50  # ⚡ Bolt: Optimización de red/IPC agrupando inserciones
...
def local_chroma_rag_inject():
    ...
    def flush_batch():
        ...
        try:
            ...
            print(f"  [X] Error fatal en lote: {e}")
        finally:
            batch_docs.clear()
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        try:
            ...
            # ⚡ Bolt: Heurística de longitud para evitar split() costoso en strings gigantes
            # Si tiene menos de 40k chars, es imposible que tenga > 40k palabras.
            if len(contenido) > 40000:
                words_peek = contenido.split(None, 40001)
                if len(words_peek) > 40000:
                    print(f"  [!] Advertencia: {archivo} es enorme. Cortando por limite interno de Chroma.")
                    contenido = " ".join(words_peek[:40000])
    ...
    # Flush final para elementos restantes
    flush_batch()
