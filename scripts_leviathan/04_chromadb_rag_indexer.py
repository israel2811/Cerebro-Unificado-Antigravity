...
# Dynamically resolve directory paths relative to the script location for system compatibility
...
def local_chroma_rag_inject():
    ...
    # Deterministic alphabetical indexing sorting
    ...
    # ⚡ OPTIMIZATION: Implement batch processing to significantly reduce index transaction overhead (Bolt Optimization)
    BATCH_SIZE = 20
    batch_docs = []
    batch_metas = []
    batch_ids = []
    ...
    def flush_batch():
        if not batch_docs:
            return
        try:
            print(f"  -> Indexing batch of {len(batch_docs)} documents...")
            # Pass copies of lists to collection.add to avoid side-effects from in-place clear()
            collection.add(
            ...
            print(f"  [X] Error vectorizing batch: {e}")
        finally:
            # Clear batch buffers in finally block to prevent "batch poisoning"
            batch_docs.clear()
            batch_metas.clear()
            batch_ids.clear()
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        # ⚡ OPTIMIZATION: Word truncation using split(None, 40001) avoids full string split overhead (Bolt Optimization)
        ...
        print(f"  -> [{i}/{len(archivos)}] Queueing: {archivo}...")
