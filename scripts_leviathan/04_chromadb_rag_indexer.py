...
def local_chroma_rag_inject():
    ...
    # BOLT OPTIMIZATION: Batch processing to reduce overhead and network calls.
    BATCH_SIZE = 20
    batch_docs = []
    batch_metadatas = []
    batch_ids = []
    ...
    def flush_batch():
        if not batch_docs:
            return
        try:
            # collection.add is much faster when called with multiple items at once.
            collection.add(
                documents=batch_docs,
                metadatas=batch_metadatas,
                ids=batch_ids
            )
            print(f"  [⚡ BOLT] Lote de {len(batch_docs)} documentos inyectado exitosamente.")
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        try:
            ...
            # BOLT OPTIMIZATION: Use split(None, 40001) to avoid splitting the entire string
            # if it's huge. This is ~130x faster for 1M word strings.
            ...
            if len(batch_docs) >= BATCH_SIZE:
                print(f"  -> [{i}/{len(archivos)}] Procesando lote (incluye {archivo})...")
                flush_batch()
            else:
                print(f"  -> [{i}/{len(archivos)}] Agregado al lote: {archivo}...")
