...
def local_chroma_rag_inject():
    ...
    # Performance optimization: Batch processing to reduce overhead
    BATCH_SIZE = 20
    batch_docs = []
    batch_metadatas = []
    batch_ids = []
    ...
    def flush_batch():
        """Sends the current batch to ChromaDB and clears the buffer."""
        if not batch_docs:
            return
        try:
            print(f"  -> Procesando lote de {len(batch_docs)} documentos...")
            collection.add(
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        # Performance optimization: Use split(None, 40001) to avoid full string splitting
