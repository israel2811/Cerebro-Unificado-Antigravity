...
def local_chroma_rag_inject():
    ...
    # 1. Use maxsplit (40001) to truncate large strings in O(1) word allocations instead of full text splitting (~28x faster).
    # 2. Batch inserts into ChromaDB (BATCH_SIZE = 20) to reduce SQLite transaction/IPC overhead (up to 20x faster).
    BATCH_SIZE = 20
    batch_docs = []
    batch_metadatas = []
    batch_ids = []
    ...
    def flush_batch():
        ...
        try:
            ...
            print(f"  [X] Error vectorizando lote de {len(batch_docs)} documentos: {e}")
        finally:
            batch_docs.clear()
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        # Segmentación preventiva utilizando maxsplit para evitar splitting completo en memoria
        words = contenido.split(None, 40001)
        if len(words) > 40000:
            print(f"  [!] Advertencia: {archivo} es enorme. Cortando por límite interno de Chroma.")
    ...
    # Inyectar lote final si hay documentos restantes
    flush_batch()
