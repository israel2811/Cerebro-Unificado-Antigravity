...
# Optimizaciones Bolt: Batching para reducir latencia de transacciones
BATCH_SIZE = 20
...
def local_chroma_rag_inject():
    ...
    # Opt Bolt: Ordenamiento determinista para consistencia de índices
    ...
    # Acumuladores para procesamiento por lotes
    batch_docs = []
    batch_metadatas = []
    batch_ids = []

    def flush_batch():
        """Envía el lote actual a ChromaDB y limpia los acumuladores."""
        if not batch_docs:
            return
        try:
            collection.add(
            ...
            print(f"  [+] Lote inyectado con éxito ({len(batch_docs)} documentos).")
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        try:
            ...
            # Opt Bolt: Truncamiento eficiente usando split(None, MAX)
            # Esto evita el escaneo doble del string y reduce el uso de memoria.
