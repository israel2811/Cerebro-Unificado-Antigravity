...
def local_chroma_rag_inject():
    ...
    # Bolt Optimization: Batch processing and efficient word truncation
    BATCH_SIZE = 50
    batch_docs = []
    batch_metadatas = []
    batch_ids = []
    ...
    def flush_batch():
        if not batch_docs:
            return
        try:
            print(f"  -> Incrustando lote de {len(batch_docs)} documentos...")
            collection.add(
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        # Bolt Optimization: Use split(None, MAX+1) to avoid full string tokenization
