...
def local_chroma_rag_inject():
    ...
    def flush_batch():
        """Envía el lote actual a ChromaDB y limpia las listas."""
        if not batch_docs:
            return
        try:
            collection.add(
            ...
            print(f"  [+] Lote de {len(batch_docs)} documentos inyectado exitosamente.")
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        try:
            ...
            # OPTIMIZACIÓN BOLT: Truncado eficiente de palabras usando maxsplit
            # Evita múltiples splits completos del string, lo cual es costoso en memoria y CPU.
            palabras = contenido.split(None, 40001)
            if len(palabras) > 40000:
                print(f"  [!] Advertencia: {archivo} excede el límite. Truncando a 40k palabras.")
            ...
            # Inyectar por lotes para reducir el overhead de llamadas a la base de datos
            if len(batch_docs) >= BATCH_SIZE:
                print(f"  -> Procesando lote hasta el archivo {i}/{len(archivos)}...")
                flush_batch()

        except Exception as e:
            print(f"  [X] Error procesando {archivo}: {e}")

    # Asegurar que los documentos restantes sean inyectados
    flush_batch()
