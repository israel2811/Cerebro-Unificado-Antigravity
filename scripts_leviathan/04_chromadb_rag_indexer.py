...
def local_chroma_rag_inject():
    ...
    # Optimización: Procesamiento por lotes (Batching) para reducir overhead de red/DB
    batch_docs = []
    batch_metadatas = []
    batch_ids = []
    BATCH_SIZE = 50
    ...
    def flush_batch():
        if not batch_ids:
            return
        try:
            print(f"  -> Inyectando lote de {len(batch_ids)} documentos en ChromaDB...")
            collection.add(
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        try:
            ...
            # Segmentación preventiva (Chroma tiene límite por lote de ~40k tokens/palabras)
            # Optimización: Usamos longitud de caracteres como pre-filtro y split limitado
