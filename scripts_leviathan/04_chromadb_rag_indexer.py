...
def local_chroma_rag_inject():
    ...
    # Sort files deterministically for consistent indexing order
    ...
    # PERFORMANCE OPTIMIZATION: Batching ChromaDB inserts (BATCH_SIZE = 20)
    # Reduces DB/Embedding overhead by vectorizing documents in batches instead of single calls.
    BATCH_SIZE = 20
    batch_docs = []
    batch_metadatas = []
    batch_ids = []
    ...
    def flush_batch():
        if not batch_docs:
            return
        try:
            print(f"  -> Incrustando lote ({len(batch_docs)} documentos)...")
            collection.add(
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        # PERFORMANCE OPTIMIZATION: Use split(None, 40001) with maxsplit
        # Avoids full-string allocation and splitting on massive files.
