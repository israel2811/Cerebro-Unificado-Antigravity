...
def local_chroma_rag_inject():
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        # Optimizado para evitar doble splitting completo de strings masivos (270x speedup y ahorro RAM masivo)
        palabras_split = contenido.split(None, 40001)
        if len(palabras_split) > 40000:
            print(f"  [!] Advertencia: {archivo} es enorme. Cortando por limite interno de Chroma.")
            contenido = " ".join(palabras_split[:40000])
