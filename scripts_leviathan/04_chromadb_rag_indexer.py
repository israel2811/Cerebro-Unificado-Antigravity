    ...
    try:
        for i, archivo in enumerate(archivos, 1):
            ...
            if len(current_batch["ids"]) >= BATCH_SIZE:
                print(f"  -> [{i}/{len(archivos)}] Incrustando lote...")
                flush_batch(current_batch, collection)

        # Finalizar cualquier documento restante
        if current_batch["ids"]:
            print(f"  -> [{len(archivos)}/{len(archivos)}] Incrustando lote final...")
            flush_batch(current_batch, collection)
    finally:
        # Asegurar que la memoria se limpie si algo falla
        current_batch["documents"].clear()
        current_batch["metadatas"].clear()
        current_batch["ids"].clear()
