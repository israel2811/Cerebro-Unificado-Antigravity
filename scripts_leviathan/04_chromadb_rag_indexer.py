...
def local_chroma_rag_inject():
    ...
    # OPTIMIZACIÓN: Procesamiento por Lotes (Batching) para reducir overhead de API/SQLite
    BATCH_SIZE = 50
    batch_docs = []
    batch_metadatas = []
    batch_ids = []
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        try:
            ...
            # OPTIMIZACIÓN: split(None, N) evita procesar todo el string si es enorme
            ...
            # Ejecutar inyección cuando el lote esté lleno
            if len(batch_docs) >= BATCH_SIZE:
                ...
                try:
                    ...
                    # Garantizar limpieza de lote para evitar "batch poisoning" en reintentos
                    batch_docs, batch_metadatas, batch_ids = [], [], []

        except Exception as e:
            print(f"  [X] Error preparando/procesando {archivo}: {e}")

    # Inyectar remanentes
    if batch_docs:
        try:
            print(f"  -> [FINAL] Inyectando lote remanente de {len(batch_docs)} documentos...")
            collection.add(
                documents=batch_docs,
                metadatas=batch_metadatas,
                ids=batch_ids
