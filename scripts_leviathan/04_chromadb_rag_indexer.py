...
def local_chroma_rag_inject():
    ...
    # Bolt: Implementación de procesamiento por lotes para reducir sobrecarga de API/IPC
    BATCH_SIZE = 20
    batch_state = {
        'docs': [],
        'metadatas': [],
    ...
    def flush_batch():
        if not batch_state['ids']:
            return
        try:
            print(f"  [*] Inyectando lote de {len(batch_state['ids'])} documentos...")
            collection.add(
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        try:
            ...
            # Bolt: Optimizado usando split(None, 40001) para evitar procesar archivos gigantes completos
