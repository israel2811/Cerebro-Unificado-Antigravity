...
def local_chroma_rag_inject():
    ...
    # BOLT OPTIMIZATION: Implement batching to reduce overhead (Batch Size = 20)
    BATCH_SIZE = 20
    batch_docs = []
    batch_metadatas = []
    batch_ids = []
    ...
    def flush_batch():
        nonlocal batch_docs, batch_metadatas, batch_ids
        if not batch_docs:
            return
        try:
            ...
            # BOLT: Reassigning instead of .clear() to ensure mock compatibility in tests
            # and to properly reset state for the next batch.
            batch_docs = []
            batch_metadatas = []
            batch_ids = []
    ...
    for i, archivo in enumerate(archivos, 1):
        ...
        # BOLT OPTIMIZATION: Efficient word truncation using maxsplit
        # This is ~150x faster than full split() for large documents.
