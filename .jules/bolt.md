...
## 2025-04-25 - Docker CI Failure
**Learning:** GitHub Actions using `docker/build-push-action` default to looking for a 'Dockerfile' in the root directory. If the Dockerfile is in `.devcontainer/`, it must be explicitly specified.
**Action:** Set `file: .devcontainer/Dockerfile` in the workflow file.
